...
변경 요청은 한 Node 프로세스에서 직렬 처리됩니다. 관련 상태·채팅·알림을 SQLite 트랜잭션으로 저장합니다. SQLite와 파일 디렉터리를 하나의 서비스 인스턴스에서 사용해야 합니다. 여러 인스턴스로 확장하려면 DB·파일 저장소·동시성 제어를 먼저 바꿔야 합니다. 신청 목록은 최근 500건, 알림은 최근 50건을 표시합니다. 채팅은 10초마다 새 내용을 조회합니다.
...
관리자 ID는 `ADMIN`이며 내부 이메일 식별자는 `admin`입니다. `ADMIN_PASSWORD` 환경변수로 최초 계정을 생성합니다. 일반 회원가입은 관리자 역할을 만들 수 없습니다. 기존 관리자가 있으면 초기값을 덮어쓰지 않습니다. 초기 관리자 비밀번호만 7자 이상을 허용하며, 신규 고객·비밀번호 변경·복구는 10~128자를 요구합니다.
